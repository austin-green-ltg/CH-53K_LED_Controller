/**
  ...
  *
  *  \brief
  *    This module contains routines to access the SPI FRAM. The FRAM used in
  *  the device is 32Kx8 with an operating frequency up to 20 MHz. It supports
  *  SPI Mode 0 & 3. The hardware supports the external Write Protect pin. The
  *  nature of this FRAM supports unlimited read/writes and data retention
  *  beyond the human life span.
  *
  *  Revision History:
 ...
 * \fn  framWriteProtect
 * -----------------------------------------------------------------------------
 * \brief This routine sets the hardware write protect pin to the correct "state".
 *
 *  \param state disable = 0, enable = 1
 *
 */
 ...
 *
 *  \param state assert = 0, release = 1
 *
 */
 ...
 *
 *  \param srP destination pointer for FRAM status register
 *
 ...
 *
 *  \param sr data value written to FRAM status register
 *
 *  \return SPI transfer status
 *
 */
 ...
 * \fn  framReadMemory
 * -----------------------------------------------------------------------------
 * \brief This routine reads FRAM memory starting at "addr" for "len" byte(s). The
 * FRAM read data is stored at the pointer referenced by "rdBufP".
 *
 *  \param addr FRAM memory address to read data
 *  \param rdBufP destination pointer to store read data
 *  \param len number of byte(s) to read
 *
 *  \return SPI transfer status
 *
 ...
 * \fn  framWriteMemory
 * -----------------------------------------------------------------------------
 * \brief This routine writes FRAM memory starting at "addr" for "len" byte(s). The
 * data referenced by the pointer "wrBufP" is written into FRAM memory.
 *
 *  \param addr FRAM memory address to read data
 *  \param wrBufP pointer to data to write
 *  \param len number of byte(s) to write
 *
 *  \return SPI transfer status
 *
 *  NOTE    : argument len must not be greater than 256
 ...
 * \fn  framTest
 * -----------------------------------------------------------------------------
 * \brief This routine is a test function for FRAM access. It writes "TLEN" bytes of
